...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(const.CONF_OUTPUT_SAFETY, default=const.DEFAULT_OUTPUT_SAFETY): vol.Coerce(
            float),
        vol.Optional(const.CONF_MIN_OUTPUT, default=const.DEFAULT_MIN_OUTPUT): vol.Coerce(float),
        vol.Optional(const.CONF_MAX_OUTPUT, default=const.DEFAULT_MAX_OUTPUT): vol.Coerce(float),
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    parameters = {
        ...
        'heater_entity_ids': config.get(const.CONF_HEATER),
        'cooler_entity_ids': config.get(const.CONF_COOLER),
        ...
        'min_output': config.get(const.CONF_MIN_OUTPUT),
        'max_output': config.get(const.CONF_MAX_OUTPUT),
...
class SmartThermostat(ClimateEntity, RestoreEntity, ABC):
    ...
    def __init__(self, **kwargs):
        ...
        self._heater_entity_ids = kwargs.get('heater_entity_ids')
        self._cooler_entity_ids = kwargs.get('cooler_entity_ids', None)
        ...
        if self._unique_id == 'none':
            self._unique_id = slugify(f"{DOMAIN}_{self._name}_{self._heater_entity_ids[0]}")
        ...
        self._min_output = kwargs.get('min_output')
        self._max_output = kwargs.get('max_output')
        ...
        self._control_output = self._min_output
    ...
    async def async_added_to_hass(self):
        ...
        if self._cooler_entity_ids is not None:
            self.async_on_remove(
                async_track_state_change(
                    self.hass,
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        if hvac_mode == HVACMode.HEAT:
            ...
            if self._pwm:
                ...
                for heater_entity_id in self._heater_entity_ids:
                    data = {ATTR_ENTITY_ID: heater_entity_id,
                    ...
                    await self.hass.services.async_call(
                        self._get_number_entity_domain(heater_entity_id),
                        SERVICE_SET_VALUE,
                        data)
                if self._cooler_entity_ids is not None:
                    for cooler_entity_id in self._cooler_entity_ids:
                        data = {ATTR_ENTITY_ID: cooler_entity_id,
                                ATTR_VALUE: self._control_output}
                        _LOGGER.debug("%s: Set cooler to %s from async_set_hvac_mode(%s)", self.entity_id,
                                    self._control_output, hvac_mode)
                        await self.hass.services.async_call(
                            self._get_number_entity_domain(cooler_entity_id),
                            SERVICE_SET_VALUE,
                            data)
            ...
            if not self._active or self._hvac_mode == HVACMode.OFF:
                if self._force_off_state and self._hvac_mode == HVACMode.OFF and \
                    ...
                    _LOGGER.debug("%s: %s is active while HVAC mode is %s. Turning it OFF.",
                                  self.entity_id, self.heater_or_cooler_entities, self._hvac_mode)
                    if self._pwm:
                        await self._async_heater_turn_off(force=True)
                    else:
                        self._control_output = self._min_output
    ...
    def _is_device_active(self):
        if self._pwm:
            ...
            if self._heater_polarity_invert:
                entities_off = any(self.hass.states.is_state(entity_id, STATE_OFF) for entity_id in
                                self.heater_or_cooler_entities)
                return entities_off
            else:
                entities_on = any(self.hass.states.is_state(entity_id, STATE_ON) for entity_id in
                                self.heater_or_cooler_entities)
                return entities_on
        else:
            """If the valve device is currently active."""
            valve_sum = sum([float(self.hass.states.get(entity_id).state) for entity_id in
                            self.heater_or_cooler_entities])
            return valve_sum > 0
    ...
    @property
    def heater_or_cooler_entities(self):
        """Return the entities to be controlled based on HVAC MODE"""
        if self.hvac_mode == HVACMode.COOL and self._cooler_entity_ids is not None:
            return self._cooler_entity_ids
        return self._heater_entity_ids

    def _set_scaled_output(self, output):
        """Set control output scaled to the output range."""
        output_ratio = output / 100.
        scaled_output = self._min_output + output_ratio * (self._max_output - self._min_output)
        self._control_output = round(scaled_output, 1)

    async def _async_heater_turn_on(self):
        """Turn heater toggleable device on."""
        for heater_or_cooler_entity in self.heater_or_cooler_entities:
            if time.time() - self._last_heat_cycle_time >= self._min_off_cycle_duration.seconds:
                data = {ATTR_ENTITY_ID: heater_or_cooler_entity}
                _LOGGER.info("%s: Turning ON %s", self.entity_id, heater_or_cooler_entity)
                if self._heater_polarity_invert:
        ...
        for heater_or_cooler_entity in self.heater_or_cooler_entities:
            if self._pwm:
                if time.time() - self._last_heat_cycle_time >= self._min_on_cycle_duration.seconds or force:
                    data = {ATTR_ENTITY_ID: heater_or_cooler_entity}
                    _LOGGER.info("%s: Turning OFF %s", self.entity_id, heater_or_cooler_entity)
                    if self._heater_polarity_invert:
                        service = SERVICE_TURN_ON
                    else:
                        service = SERVICE_TURN_OFF
                ...
                _LOGGER.info("%s: Change state of %s to %s", self.entity_id,
                            heater_or_cooler_entity, self._min_output)
                # self.hass.states.async_set(self._heater_entity_id, self._control_output)
                data = {ATTR_ENTITY_ID: heater_or_cooler_entity, ATTR_VALUE: self._min_output}
                await self.hass.services.async_call(
    ...
    async def calc_output(self):
        ...
        if self._autotune != "none":
            if self._trigger_source == "sensor":
                ...
                if self._pid_autotune.run(self._current_temp, self._target_temp):
                    ...
                    self._pid_controller = pid_controller.PID(self._kp, self._ki, self._kd,
                                                              ...
                                                              self._hot_tolerance)
                    self._autotune = "none"
            self._set_scaled_output(self._pid_autotune.output)
            self._p = self._i = self._d = error = self._dt = 0
        else:
            if self._pid_controller.sampling_period == 0:
                output, update = self._pid_controller.calc(self._current_temp,
                                                                         self._target_temp,
                                                                         self._cur_temp_time,
            ...
            self._set_scaled_output(output)
                ...
                if abs(self._control_output) == self._difference:
                    if not self._is_device_active:
                        _LOGGER.info("%s: Output is %s. Request turning ON %s", self.entity_id,
                                    self._difference, heater_or_cooler_entity)
            ...
            if self._is_device_active:
                if time_on <= time_passed or self._force_off:
                    ...
                    _LOGGER.info("%s: Time until %s turns OFF: %s sec", self.entity_id,
                                heater_or_cooler_entity, int(time_on - time_passed))
            else:
                if time_off <= time_passed or self._force_on:
                    ...
                    _LOGGER.info("%s: Time until %s turns ON: %s sec", self.entity_id,
                                heater_or_cooler_entity, int(time_off - time_passed))
            self._force_on = False
            self._force_off = False
